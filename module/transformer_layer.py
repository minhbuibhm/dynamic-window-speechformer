...
class Multihead_attention(nn.Module):
    ...
    def forward(self, query, key=None, value=None, key_padding_mask=None, attn_mask=None, need_weights=False):
        ...
        softmax_attn_output_weights = F.softmax(attn_output_weights, dim=-1)
        dropout_attn_output_weights = F.dropout(softmax_attn_output_weights, p=self.dropout, training=self.training)

        attn_output = torch.bmm(dropout_attn_output_weights, V)
        ...
        if need_weights:
            ...
            # return attn_output, (attn_output_weights.sum(dim=1) / self.num_heads).sum(dim=1)
        else:
