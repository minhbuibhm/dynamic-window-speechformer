...
class Engine():
    ...
    def train_epoch(self):
        logger.info(f"<class> Engine: train_epoch()")
    ...
    def run(self):
        logger.info(f"<class> Engine: run!")
        if self.cfg.train.find_init_lr:
...
def main_worker(local_rank, cfg, world_size, dist_url):
    logger.info(f"<class> Engine: main_worker()")
    # 31/3 fix start
    ...
    logger.info(f"<class> Engine - main_worker(): folds = {folds}")
    for f in folds:
        cfg_clone = cfg.clone()
    ...
    logger.info(f"<class> Engine - main_worker(): store result")
    if local_rank == 0:
...
def main(cfg):
    ...
    print(f'Using dist_url={dist_url}')
